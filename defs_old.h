...
static void copy2dev(GeoParams &hostP, GeoParams &devP) {
  for(int i=0; i<NDev; i++) {
    CHECK_ERROR( cudaSetDevice(i) );
    CHECK_ERROR( cudaMemcpyToSymbol(devP, &hostP, sizeof(GeoParams)) );
  }
...
#define cPLASTER(x,y)   rc[(x)*NStripe(idevC)]
#define mPLASTER(x,y)   rm[(x)*NStripe(idevM)]
#define pPLASTER(x,y)   rp[(x)*NStripe(idevP)]
#define cPLASTER_A(x,y) rcPMLa[(x)*Npmly]
#define cPLASTER_S(x,y) rcPMLs[(x)*NStripe(idevC)]
...
//#define TEXCOFFV(xt,yt,z,I,h) coffV = tex3D<float >(pars.texs.layerV[idevC][I],h*texStretchH, (z)*texStretchY, GLOBAL(xt)*texStretch[I]); /*if(threadIdx.x==Nz/2) printf("coffV(%d %d %d)=%g\n", xt,z,h, coffV);*/
//#define TEXCOFFT(xt,yt,z,I,h) coffT = tex3D<float >(pars.texs.layerT[idevC][I],h*texStretchH, (z)*texStretchY, GLOBAL(xt)*texStretch[I]); /*if(threadIdx.x==Nz/2) printf("coffT(%d %d %d)=%g\n", xt,z,h, coffT);*/
//#define TEXCOFFS(xt,yt,z,I,h) coffS = tex3D<float2>(pars.texs.layerS[idevC][I],h*texStretchH, (z)*texStretchY, GLOBAL(xt)*texStretch[I]); /*if(threadIdx.x==Nz/2) printf("coffS(%d %d %d)=%g,%g\n", xt,z,h, coffS.x, coffS.y);*/
#define TEXCOFFV(xt,yt,z,I,h) coffV = tex3D<float >(pars.texs.TexlayerV[curDev], (z)*texStretchY, 0*texStretchH, GLOBAL(xt)*texStretch[0]); /*if(threadIdx.x==Nz/2) printf("coffV(%d %d %d)=%g\n", xt,z,h, coffV);*/
#define TEXCOFFT(xt,yt,z,I,h) coffT = tex3D<float >(pars.texs.TexlayerT[curDev], (z)*texStretchY, 0*texStretchH, GLOBAL(xt)*texStretch[0]); /*if(threadIdx.x==Nz/2) printf("coffT(%d %d %d)=%g\n", xt,z,h, coffT);*/
#define TEXCOFFS(xt,yt,z,I,h) coffS = tex3D<float2>(pars.texs.TexlayerS[curDev], (z)*texStretchY, 0*texStretchH, GLOBAL(xt)*texStretch[0]); /*if(threadIdx.x==Nz/2) printf("coffS(%d %d %d)=%g,%g\n", xt,z,h, coffS.x, coffS.y);*/
...
#define REG_DEC(EVENTYPE) \
  ...
  int it=t0; ftype difx0,dify0,difz0; ftype zerov=0.;\
...
#define RPOINT_CHUNK_HEAD\
  ...
  if(chunkSi[1]-chunkSi[0]>0) { channelSx[0]=uint32_t(pars.iStep*Ntime+it); channelSx[1]=uint32_t(glob_ix*NDT+chunkSi[0]); channelSx[2]=uint32_t(glob_ix*NDT+chunkSi[1]); }\
  if(chunkSi[1]-chunkSi[0]>0) { channelSy[0]=uint32_t(pars.iStep*Ntime+it); channelSy[1]=uint32_t(glob_ix*NDT+chunkSi[0]); channelSy[2]=uint32_t(glob_ix*NDT+chunkSi[1]); }\
  if(chunkSi[1]-chunkSi[0]>0) { channelSz[0]=uint32_t(pars.iStep*Ntime+it); channelSz[1]=uint32_t(glob_ix*NDT+chunkSi[0]); channelSz[2]=uint32_t(glob_ix*NDT+chunkSi[1]); }\
  if(chunkTx[1]-chunkTx[0]>0) { channelTx[0]=uint32_t(pars.iStep*Ntime+it); channelTx[1]=uint32_t(glob_ix*NDT+chunkTx[0]); channelTx[2]=uint32_t(glob_ix*NDT+chunkTx[1]); }\
  if(chunkTy[1]-chunkTy[0]>0) { channelTy[0]=uint32_t(pars.iStep*Ntime+it); channelTy[1]=uint32_t(glob_ix*NDT+chunkTy[0]); channelTy[2]=uint32_t(glob_ix*NDT+chunkTy[1]); }\
  if(chunkTz[1]-chunkTz[0]>0) { channelTz[0]=uint32_t(pars.iStep*Ntime+it); channelTz[1]=uint32_t(glob_ix*NDT+chunkTz[0]); channelTz[2]=uint32_t(glob_ix*NDT+chunkTz[1]); }\
  if(chunkVx[1]-chunkVx[0]>0) { channelVx[0]=uint32_t(pars.iStep*Ntime+it); channelVx[1]=uint32_t(glob_ix*NDT+chunkVx[0]); channelVx[2]=uint32_t(glob_ix*NDT+chunkVx[1]); }\
  if(chunkVy[1]-chunkVy[0]>0) { channelVy[0]=uint32_t(pars.iStep*Ntime+it); channelVy[1]=uint32_t(glob_ix*NDT+chunkVy[0]); channelVy[2]=uint32_t(glob_ix*NDT+chunkVy[1]); }\
  if(chunkVz[1]-chunkVz[0]>0) { channelVz[0]=uint32_t(pars.iStep*Ntime+it); channelVz[1]=uint32_t(glob_ix*NDT+chunkVz[0]); channelVz[2]=uint32_t(glob_ix*NDT+chunkVz[1]); }\
  }\
...
#define RPOINT_CHUNK_SHIFT\
  for(int xdrop=chunkSi[0]; xdrop<chunkSi[1]; xdrop++) for(int iwarp=0; iwarp<Nz/WSIZE; iwarp++) channelSx+= __popc(drop_cells[(ix*NDT+xdrop)%(Ns*NDT)*Nwarps+iwarp]);\
  for(int xdrop=chunkSi[0]; xdrop<chunkSi[1]; xdrop++) for(int iwarp=0; iwarp<Nz/WSIZE; iwarp++) channelSy+= __popc(drop_cells[(ix*NDT+xdrop)%(Ns*NDT)*Nwarps+iwarp]);\
  for(int xdrop=chunkSi[0]; xdrop<chunkSi[1]; xdrop++) for(int iwarp=0; iwarp<Nz/WSIZE; iwarp++) channelSz+= __popc(drop_cells[(ix*NDT+xdrop)%(Ns*NDT)*Nwarps+iwarp]);\
  for(int xdrop=chunkTx[0]; xdrop<chunkTx[1]; xdrop++) for(int iwarp=0; iwarp<Nz/WSIZE; iwarp++) channelTx+= __popc(drop_cells[(ix*NDT+xdrop)%(Ns*NDT)*Nwarps+iwarp]);\
  for(int xdrop=chunkTy[0]; xdrop<chunkTy[1]; xdrop++) for(int iwarp=0; iwarp<Nz/WSIZE; iwarp++) channelTy+= __popc(drop_cells[(ix*NDT+xdrop)%(Ns*NDT)*Nwarps+iwarp]);\
  for(int xdrop=chunkTz[0]; xdrop<chunkTz[1]; xdrop++) for(int iwarp=0; iwarp<Nz/WSIZE; iwarp++) channelTz+= __popc(drop_cells[(ix*NDT+xdrop)%(Ns*NDT)*Nwarps+iwarp]);\
  for(int xdrop=chunkVx[0]; xdrop<chunkVx[1]; xdrop++) for(int iwarp=0; iwarp<Nz/WSIZE; iwarp++) channelVx+= __popc(drop_cells[(ix*NDT+xdrop)%(Ns*NDT)*Nwarps+iwarp]);\
  for(int xdrop=chunkVy[0]; xdrop<chunkVy[1]; xdrop++) for(int iwarp=0; iwarp<Nz/WSIZE; iwarp++) channelVy+= __popc(drop_cells[(ix*NDT+xdrop)%(Ns*NDT)*Nwarps+iwarp]);\
  for(int xdrop=chunkVz[0]; xdrop<chunkVz[1]; xdrop++) for(int iwarp=0; iwarp<Nz/WSIZE; iwarp++) channelVz+= __popc(drop_cells[(ix*NDT+xdrop)%(Ns*NDT)*Nwarps+iwarp]);\
  if(threadIdx.x==blockDim.x-1) {\
